...
interface OnBarcodeEvent {
  barcode: string;
}
type NativeCameraViewProps = Omit<
  CameraProps,
  'device' | 'onInitialized'| 'onBarcodeScaned'| 'onError'  | 'frameProcessor' | 'frameProcessorFps'
> & {
  cameraId: string;
  frameProcessorFps?: number; // native cannot use number | string, so we use '-1' for 'auto'
  onInitialized?: (event: NativeSyntheticEvent<void>) => void;
  onError?: (event: NativeSyntheticEvent<OnErrorEvent>) => void;
  onBarcodeScaned?: (event:NativeSyntheticEvent<OnBarcodeEvent>) => void;
  onViewReady: () => void;
};
...
const CameraModule = NativeModules.BarcodeView;
if (CameraModule == null) console.error("Camera: Native Module 'BarcodeView' was null! Did you run pod install?");
...
export class Camera extends React.PureComponent<CameraProps> {
  ...
  constructor(props: CameraProps) {
    ...
    this.onBarcodeScaned=this.onBarcodeScaned.bind(this);
  ...
  public static async getCameraPermissionStatus(): Promise<CameraPermissionStatus> {
    try {
      let t= await CameraModule.getCameraPermissionStatus();
      return t;
    } catch (e) {
      throw tryParseNativeCameraError(e);
    }
  ...
  private onBarcodeScaned(event: NativeSyntheticEvent<OnBarcodeEvent>): void {
    if(this.props.onBarcodeScaned!=null){
      this.props.onBarcodeScaned(event.nativeEvent.barcode);
    }
  }
  ...
  public render(): React.ReactNode {
    ...
    return (
      <NativeCameraView
        ...
        onBarcodeScaned={this.onBarcodeScaned}
      />
    );
  }
}
