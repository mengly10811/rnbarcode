...
import com.barcode.CameraView
import com.barcode.ViewNotFoundError
import java.util.concurrent.ExecutorService
import com.barcode.parsers.*
import com.barcode.utils.*
import kotlinx.coroutines.*
import kotlinx.coroutines.guava.await
import java.util.concurrent.Executors

@ReactModule(name = BarcodeViewModule.TAG)
@Suppress("unused")
class BarcodeViewModule(reactContext: ReactApplicationContext):ReactContextBaseJavaModule(reactContext){
    companion object {
    ...
    coroutineScope.launch {
      withPromise(promise) {
        ...
        manager.cameraIdList.filter{ id -> id.toIntOrNull() != null }.forEach loop@{ id ->
         ...
         // val isMultiCam = Build.VERSION.SDK_INT >= Build.VERSION_CODES.P &&capabilities.contains(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_LOGICAL_MULTI_CAMERA)
         ...
         // val supportsDepthCapture = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && capabilities.contains(CameraCharacteristics.REQUEST_AVAILABLE_CAPABILITIES_DEPTH_OUTPUT)
        ...
        //   val zoomRange = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R)characteristics.get(CameraCharacteristics.CONTROL_ZOOM_RATIO_RANGE)
        //   else null
        ...
        //   else null
          ...
          map.putString("name", "${parseLensFacing(lensFacing)} ($id)")
